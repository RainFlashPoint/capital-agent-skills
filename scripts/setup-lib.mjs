...
当当前目录位于 Git 仓库中，且用户意图是实现功能、修复问题、重构、补测试、代码评审或发布交付时，必须在实质分析或修改前加载已安装的 \`cap\` Skill。先以用户当前打开的仓库（或本轮首次明确指定的目标仓）运行 \`scripts/cap-status.mjs\`，锁定本会话 canonical Git root；旧 \`.cap\` 中的 worktree 或绝对路径只用于校验，绝不能反向选择另一个目录。检查当前宿主是否实际暴露 Capital Agent MCP 工具，并把结果作为 \`loaded\` / \`missing\` 传给 \`scripts/cap-status.mjs --mcp-runtime\`。若返回 \`session_root_blocked\`，停止读取错误仓并回到锁定仓库；确需切换 worktree 时新建会话。若返回 \`restart_required\`，先提示用户选择“重启后使用团队模式”或“本次明确改用本地模式继续”；用户选择本地继续后，使用 \`--allow-local-once\` 重跑状态检查，本任务不创建平台 Task、不回写经验或 Server Gate，但不修改机器的团队模式配置。若 \`CAPITAL_AGENT_MODE=local\`，按纯本地流程完成代码侦察、实现和本地证据，不连接平台、不创建 Task、不写 Outbox；否则按 Skill 要求完成平台握手、创建或绑定统一 Task、实现和证据回写。不要等待用户显式输入 \`$cap\` 或 \`/cap\`。
