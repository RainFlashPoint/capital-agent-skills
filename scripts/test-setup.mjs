...
test('installs native HTTP Codex MCP config without persisting the user key or replacing other config', async () => {
  ...
  assert.match(content,/required = false/)
  assert.doesNotMatch(content,/required = true/)
...
test('upgrade migrates a managed required Codex MCP entry to optional startup', async () => {
  const home = await mkdtemp(join(tmpdir(),'cap-codex-required-migration-'))
  const helper = join(home,'repo','mcp-http-headers.mjs'); await mkdir(join(home,'repo'),{recursive:true}); await writeFile(helper,'// fixture\n')
  const target = codexConfigPath(home); await mkdir(join(home,'.codex'),{recursive:true})
  await writeFile(target,`# capital-agent:mcp:start\n[mcp_servers.capital-agent]\nurl = "https://old.example.test/api/mcp/message"\nhttp_headers_helper = "old"\nrequired = true\n# capital-agent:mcp:end\n`)
  await installCodexMcpConfig(target,'https://example.test','/opt/node',helper)
  const content = await readFile(target,'utf8')
  assert.equal(content.split('[mcp_servers.capital-agent]').length-1,1)
  assert.match(content,/required = false/)
  assert.doesNotMatch(content,/required = true|old\.example\.test/)
  assert.equal((await inspectCodexMcpConfig(target,'https://example.test',helper)).valid,true)
})
test('doctor accepts the native HTTP transport and rejects stale local or proxy stdio transports', async () => {
  ...
  await writeFile(target,`[mcp_servers.capital-agent]\nurl = "https://example.test/api/mcp/message"\nhttp_headers_helper = ${JSON.stringify(`"/opt/node" "${helper}"`)}\nrequired = true\n`)
  const required = await inspectCodexMcpConfig(target,'https://example.test',helper)
  assert.equal(required.transport,'streamable-http')
  assert.equal(required.required,true)
  assert.equal(required.valid,false)
  assert.equal(required.reason,'required_startup_blocks_session')
...
test('installation manifest records source commit, version and managed file hashes, then doctor detects drift', async () => {
  ...
  assert.ok(installed.recorded.files.some(item => item.path === 'ontology/README.md' && item.sha256))
  ...
  await writeFile(join(source, 'ontology', 'README.md'), '# stale ontology\n')
  const ontologyDrift = await inspectInstallManifest(home, source)
  assert.equal(ontologyDrift.reason, 'file_manifest_drift')
  assert.deepEqual(ontologyDrift.changedFiles, ['ontology/README.md', 'skills/cap/SKILL.md'])
})
