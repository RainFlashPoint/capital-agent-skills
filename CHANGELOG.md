...
- 验证证据必须绑定候选 Commit：失败命令不可声明 PASS，命令仅上传哈希，环境指纹与质量资产采用严格白名单；canonical 回读必须确认同一 Task 与候选。
- Push 成功后安全刷新本地远端跟踪分支；远端漂移与 Git 错误只返回脱敏摘要，拒绝嵌入凭据的 fetch/push URL；本机 Test Provider 如实声明 `public` 网络区。
