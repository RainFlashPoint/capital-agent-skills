...
owner: <本特性的人类归属：当初 shape/plan 拍板的人；缺省 (none)>   # 归因用；退场随数据点吐出
...
- [ ] map：PROFILE.md 已建立 / 已确认无漂移
- [ ] shape：spec.md 已获批（含 AI 工作的 eval 标准，若适用）
- [ ] plan：plan.md 已拆分（阶段含依赖、任务含三字段）
- [ ] build：tests written (red) — 测试先行且确认失败
- [ ] build：implementation (green) — 实现使测试通过
- [ ] verify：logic 通过（套件 + 覆盖率门控）
- [ ] verify：journey 通过（用户旅程，若有可见面变更）
- [ ] verify：model 通过（达 rubric/阈值，若有 AI 变更）
