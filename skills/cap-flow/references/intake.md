...
`--gate-kind server`，脚本必须实时读回已认证的 Server canonical Task，并确认同 Task/同 Commit 的 Review Action
`succeeded`；`local` / `local-only` 使用 `--gate-kind local`，脚本必须从同 Task `STATE.md` 读到精确
`cap-gate: PASS reviewed-head=<delivery commit>`。两类证明不能互相冒充，单独传入 `--gate-status passed`
不构成证明；`--gate-commit` 必须等于完整
...
严格退场必须显式声明知识处置，并先证明 `experience.md` 的 `task-id` 与完整 `source-commit` 精确对应当前 Task 和 Delivery Commit；可复用处置还必须至少有一个有效代码路径、入口或符号锚点，`N/A` / `none` / `unknown` / `无` 等占位值不算锚点：`synced` 必须带中心知识文档 ID，`pending-sync` 必须有同 Task、同 Commit、envelope/payload 幂等键一致且包含完整结构化经验载荷的可重放
...
`python3 scripts/intake.py knowledge-audit --cap <target>/.cap` 报告；审计对索引与 `EVOLUTION.md` 共享总读取字节硬预算，
`EVOLUTION.md` 本身也必须是 no-follow、有界普通文件，并显式输出 `scanned / truncated / overBudget / limits / total`，
避免经验静默丢失或长期仓库增长导致无界读取。PROFILE.md 不承载流水,
