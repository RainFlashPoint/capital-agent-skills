...
1. cap-review 侧标出"复发"：同 `category` 必须来自至少 3 个不同 Task/Run；单次 Review 内多个同类 finding 只算一次。
...
生命周期唯一为 `candidate → validated → promoted → deprecated`。validated 只在当前项目可见；promoted 必须累计至少两个不同 Task 的独立证据并由管理员批准；错误、过期或冲突经验必须 deprecated，记录原因和可选替代文档 ID，废弃后不得注入。相同 `problem + solution` 使用稳定 fingerprint 去重，重复提交合并证据而不是复制经验卡。

外部 Skill 只有在 `source_url`、`source_version`、`license` 与 fixture/验证报告引用齐全时才能 validated；来源或许可证不明时保持 candidate。
...
- **中心 KB 条目**:靠 `operator` + `repo_url` 归因——谁贡献、来自哪个项目。这是曝光率、采纳率与 F1
...
origin`),harvest-experience 首尾两次调用都用它。`exposure_rate` 只表示被展示，`adoption_rate` 才表示被采用，`misleading_rate` 反映知识误导；不要把曝光率表述成真实复用率。
