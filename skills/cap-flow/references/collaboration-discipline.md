---
title: 源码协作纪律（分支 / 命名 / 提交 / worktree / 并行收敛）
scope: methodology                # 通用、跨语言、跨目标——不含项目特定值
applies-to: cap-flow(driver §1.1, 跨阶段) + cap-implement §0.2   # driver 在 entry 加载、贯穿 plan→build→review;build 用于提交/波内执行
updated: 2026-08-06
---

# 源码协作纪律 · collaboration-discipline

> 通用方法论卡:回答"多人/多 agent 共享一个代码库怎么不打架"。技术栈无关。
> 第一性原理:① 集成成本随分叉时长增长;② 并行的代价是把集成推迟;③ 隔离的 agent 之间**没有共享上下文** → 协调必须**前置**。本卡只给约束与原则,具体命令用稳定 git 原语示意。

---

## 0. 任务入口：分支意图 Gate

任何新 feature / fix / hotfix 在写 spec、plan 或代码前，先判断“当前分支是不是为本任务准备的”，不能只判断它是否叫 `main`：

1. 读取当前分支、默认主干、HEAD、与默认主干的共同基线，以及现有 STATE / Task 是否与本次意图一致。
2. 当前为默认主干，或当前分支属于历史功能、release、客户定制、维护版本、其他活动 Task，而本次是新的收敛型工作 → **停在 Gate**，用编号文本让用户选择：
   - 从最新默认主干创建推荐短命分支；
   - 明确继续当前分支，并说明它为什么就是正确基线；
   - 切换到用户指定的维护 / 变体分支。
3. 当前已有未提交改动时，先列出路径与归属；不能为了切分支自动 stash、搬运或提交用户改动。改动与本任务无关且会阻碍切换时，停下让用户处理或改用 worktree。
4. 决定后把 `branch-purpose / base-branch / base-commit` 写进 STATE；后续分支、基线或任务意图变化即使 guard 尚未报错，也必须重新过 Gate。

推荐提示：

```text
当前分支：feature/old-payment（历史任务）
本次意图：新增退款审批
→ 分支与任务不匹配。
  1) 从最新 origin/main 创建 feature/refund-approval（推荐）
  2) 继续当前分支，并记录原因
  3) 切到指定维护分支
```

不要机械地每次都提问：当前短命分支已由同一 STATE / Task 绑定本次意图且基线未漂移时，直接续接。

## 1. 分支模型(按意图分两类,不一刀切)

"短命"只在**目标是收敛(要合回主干)**时成立。先判:这条分支要不要并回主干。

- **A 收敛型(feature/fix/hotfix)= 短命默认**
  - 从最新主干切短命分支(意识 ≤1–2 天),频繁集成回主干;主干**永远可发布**。
  - 大改用**特性开关 + branch-by-abstraction** 保持短命,不开长命分支攒合并债。
- **B 分叉型(客户定制 / 私有部署 / 长期维护版 / 上游 fork)= 长命是设计,不是失误**
  - ① **能用配置+扩展点表达就别 fork**(同一制品+私有配置,见 `deployment-patterns §7`)。分叉是最后手段。
  - ② 必须分叉则**压小 delta**、把定制隔离进清晰模块/overlay。
  - ③ 同步**单向 核心→变体**;某条定制变通用 → **提升回核心**(走一次普通短命特性)再从变体删。
  - ④ 明确 owner + 支持周期(EOL),只收 backport,不攒新特性。
- 反模式:用长命分支代表环境(改用配置晋级,见 `deployment-patterns §1`);收敛型分支活太久。
- 判据:`要并回主干 → A;不并/不能 → B`。

> **cap 执行段产物必走隔离分支(A 型的强制特例)**:`/cap loop` 及一切无头/夜间自治跑,build→verify→review
> 的产物**一律落 `cap/<leaf-id>` 短命分支**(从最新主干切),收敛后**开 PR 交人审**,**绝不 commit / push main**——
> main 始终可发布,夜里跑错最坏只是一条待关 PR。这是"晚上安全地多产出、早上人放行"的结构基础;无头下的分支+PR
> 协议、缺 PR 通道的降级、runner 归因见 `headless-policy.md §1/§3`。

## 2. 分支命名

- 形如 `<type>/<summary>`,type 取 `feature/fix/hotfix/chore/refactor/docs`(推荐默认;项目可自定但**一仓内统一**)。
- 分支 type 与提交 type **别混**(分支 `feature/`,提交 `feat:`)。
- 项目已有明确命名规范时以项目规范为准；无规范时由 Cap 根据意图给出建议名，不悄悄复用一个“看起来能用”的历史分支。

## 3. 提交信息

- Conventional Commits:`<type>(<scope>): <subject>`;中文 subject 允许。
- **研发产物与代码同交付**：提交包含业务代码时，当前特性的 `.cap/STATE.md`、spec、plan、verify、review 等新增或修改产物必须一并暂存提交；不得用 `.git/info/exclude` 或仓库 `.gitignore` 隐藏 `.cap/`。本地 runtime、日志、缓存和密钥继续按 `.cap/.gitignore` 排除。
- Capital Agent Hook 自动从 `.cap/STATE.md` 追加 `Task:` / `Session:` trailer。提交完成后以 `git show -s --format='%B' HEAD` 为准确认，不能只在聊天里声称已关联。
- 本地 commit 只代表本机完成；只有 push 后 GitLab Webhook/轮询才能触发平台 Review/Test/钉钉闭环。交接必须明确区分“已提交未推送”和“已推送平台可见”。
- 有外部消费者的制品用 SemVer(版本/制品规范见 `deployment-patterns`)。

### 3.1 提交范围 Gate

准备暂存或提交前，必须基于 `git status`、本任务开始时的脏区快照、plan 预测面和实际 diff 生成一次 **Commit scope manifest**，按三类展示：

- **必须提交**：本任务实现、回归测试、迁移、对外契约文档、必要的共享配置，以及本任务应交付的 `.cap` 产物。
- **需要确认**：`docs/`、仓库级测试 / lint / build 配置、lockfile、生成物、超出 plan 的文件，以及无法证明属于本任务的既有改动。不能按目录名一刀切；与行为或契约相关就应提交，本机临时调整则应排除。
- **明确排除**：密钥、`.env` 本机值、缓存、日志、临时调试文件、本地 IDE / 工具配置，以及确认属于其他任务的改动。

只有“需要确认”非空、存在删除 / 大范围配置变化、或用户尚未授权提交时才停下来问；其余情况展示清单后按已授权动作继续。暂存时使用 manifest 中的明确路径，禁止用 `git add .`、`git add -A` 把整个脏区一锅端。

测试和文档的默认判断：

- 能证明本次行为的回归测试通常是交付物，不能以“测试文件”为由排除。
- 描述新接口、配置、迁移或用户行为的文档通常随变更提交。
- 通用测试配置若改变团队 / CI 行为，应提交并明确影响；只为本机临时跑通而改的配置不得混入。

Commit scope manifest 写入 STATE 的 `## Commit scope` 作为交接快照；diff 变化后快照立即过期，提交前必须重算。

## 4. worktree —— 按需,不是默认

worktree 解决的是"**同时进行且互不干扰**"。没有并行需求就**别开**,普通分支够用。

- **该开**:并行多个**真正独立**的工作(多特性 / 多 agent 同时干);一个长任务想与主线隔离;需同时检出两分支对比联调。
- **不必开**:小修一次性改、串行单线、与在飞改动碰同一批文件的紧耦合、磁盘/配额紧。
- **审慎**:并行有隐性成本——隔离的 agent 各自做**隐式决策、会冲突**。默认少开、能单线就单线。
- 开了就守:一任务一分支一目录、命名 `<repo>-<purpose>`、**各跑各的 DB/端口**、完事即清理;并发 **3–5 上限**。

开 worktree 命令骨架(git worktree 是稳定原语):
```bash
git fetch origin
git worktree add ../<repo>-<purpose> -b <type>/<summary> origin/main   # 从最新主干切
#   再配本目录独立 .env / DB(或 schema) / 端口;初始化各自 .cap 状态。合见 §5.3
```

## 5. 并行协作:前置合约(核心)+ 收敛安全网

隔离的 agent 没有共享上下文 → 不在开之前把"怎么对齐"定死,各方会做出**局部合理、彼此不兼容**的选择(上下文对不齐)。**省事在前置,后置只是兜底。**

### 5.1 前置:开 worktree 之前定死

- **独立性测试**:只对**真正独立**的子任务开并行;B 依赖 A → 串行(并行只会变成带额外开销的串行)。
- **契约先冻**:agent 间共享的接口 / schema / 事件契约**先定死**,作各方单一事实源(接口对齐是消除"对不齐"的头号杠杆;跨边界契约审查见 `roles/architect`)。
- **面切分 + 文件归属**:谁动哪些模块/文件**不重叠**;"无主之地"显式归谁。会碰同一批文件 → 别并行。
- **同一基**:都从**同一 commit(最新主干)**切。
- **自包含简报 + 显式边界**:每个 agent 给「目标 + 产出格式 + 明确"别碰 X,那是别人的活"」。
- **决策落共享记忆**:关键决策写进各自 `STATE` / 汇进共享记忆,不靠 agent 互相"看见"(复用 cap 的 STATE 交接,不另造)。

### 5.2 实施中

- 保持短命、聚焦本面;每个 checkpoint 后**从主干同步进来**,让漂移小、冲突早暴露而非攒到最后。
- 越界即停:发现必须动别人那一面 → 前置合约破了,**先改合约,别私改**。

### 5.3 后置:收敛安全网(残余漂移兜底)

即便有合约,基线仍会动。**收敛是串行的**:每次合并都改变其余分支的目标基。

- **串行收敛**:合主干一次一个,经单一闸口,不并发抢合。
- **★先基到最新 + 重测,才合**:合 B 前把 B 重基到含 A 的最新主干、**重跑全量验证**(绿要相对最新基),再合。这条防 **merge skew**——孤立看兼容、合进更新后的主干就坏。
- **合后跑全量**:抓**语义冲突**(文本无冲突 ≠ 正确)。
- **冲突在特性分支里解**(rebase 到主干上解),保主干始终可发布。
- **生成物不手合**:lockfile / 生成客户端 / 快照 → 取一边后**重新生成**,别逐行合噪音。
- **迁移对齐**:加法式(expand)迁移对顺序不敏感(见 `deployment-patterns §3`)。
- **收尾清场**:合完删分支 + `git worktree remove` + `prune` + 拆隔离资源;无孤儿。

合并命令骨架:
```bash
# 1) worktree 内:基到最新主干,冲突在分支内解(保主干干净)
git fetch origin && git rebase origin/main
# 2) 重跑全量验证(绿相对最新基,不是旧基)
# 3) 串行落主干,一次一个(squash 默认;需 bisect 友好可 merge-commit,一仓统一)
git switch main && git merge --squash <type>/<summary> && git commit
# 4) 其余在飞 worktree 立刻重基到新主干(别继续偏离)
# 5) 合后在主干跑全量,抓语义冲突
# 6) 清场
git worktree remove ../<repo>-<purpose> && git branch -d <type>/<summary> && git worktree prune
```

> 有 CI 时,§5.3 的"先基+重测"可由 merge queue 自动化;无 CI 则作人工纪律。

### 5.4 监督式多-agent build loop(自治并行的高烈度形态)

把 §5 的并行交给**后台 / 自治 agent**(主循环只监督、不在每步等人确认)时,§5.1 的前置要再收紧、§5.3 的收敛要多一道闸——这是同一纪律在"**agent 中途问不了人**"约束下的强化版,不是新方法。

**loop 形状(串行冻 → 并行建 → 集成收敛):**

```
串行(人在环):  冻决策 + 冻契约 → 独立评审契约(对抗式,可多轮)
   ▼
并行(后台 agent,主循环监督):每 track 一 worktree,摸不重叠的面,各自跑到绿
   ▼
串行(主循环):  合并 → ★跨 track 集成闸 → 定向 remediation → 复审 → 收口
```

关键:**不是"撒 N 个 agent 各自跑完整 cap 流程"**(各自猜、合不拢),而是"**串行冻契约 → 并行建竖切 → 主循环跑一次集成 cap 流程收敛**"。

**前置再收紧(补 §5.1)—— 自治 agent 无人工通道:**

- 后台 agent 遇模糊点**没法问你、只会自己猜**。所以冻死的不只是接口契约,而是**所有会让 agent 现场拍板的决策**(架构取舍 / 边界口径 / 取值语义)——派发前在串行段(人在环)全部敲定并落成文件。
- 每个 agent 简报必含一句硬约束:**"契约 / 边界之外受阻 → 停下回报,不要猜。"** 把"猜"改成"停",让漂移在派发后第一时间暴露,而非埋进产物到集成时才炸。
- 真栈并行的 agent 各自**自举隔离环境**(独立 DB/schema + 端口 + 缓存 db + 依赖安装),否则并发 e2e 互撞(呼应 §4 worktree 纪律)。

**收敛再加闸(补 §5.3)—— 契约解读漂移,per-track 测试看不见:**

- ★**冻结的契约只保证"形状",不保证"解读"**:并行 track 会对同一共享边界做出**各自局部自洽、彼此不兼容**的解读(路径如何 normalize、字段顺序、canonical 串拼法、空 body 的 hash)。每条 track 的单测都**对着自己那套解读**跑、且都绿——所以 per-track 测试**根本测不出**这种跨 track 解读漂移。
  - 实证:一侧签名用 `/suppliers`、另一侧验签用 `/vendor/api/v1/suppliers` → canonical 不一致 → **真实请求 100% 401(flow-dead)**;两侧单测全绿,整条链路却不通。
- 故收敛**必过一道跨 track 集成闸**:由**独立评审者**(新 agent / 第二模型,喂**两端真实接线**而非各 track 的自述摘要)审"边界两侧是否真对齐",或跑**一条真穿过那条缝的 e2e**。这是 §5.3"合后跑全量抓语义冲突"的尖锐特例——专盯**共享边界的解读一致性**。
- 闸是**对抗式 + 迭代**的:评审 → 定向 remediation → 复审,直到收敛(契约阶段、集成阶段各可能多轮);别指望一轮过。

一句话(5.4):**自治并行 = 决策全部前移到冻结点 + 边界对齐后移到一道跨 track 集成闸;agent 问不了人,所以"猜"要改"停",per-track 绿 ≠ 集成通。**

---

一句话:**并行地做、串行地合;先定合约再开 worktree;每次合前必"基到最新+重测",合后必跑全量。**
