...
role: server-dev
updated: 2026-07-26
distilled-from: session:external-integration-safety-invariants-2026-07-26
triggers:
  - "**/api/**"
  - "**/handlers/**"
  - "**/services/**"
...
### E. External Integration Safety Invariants

任何能够调用外部系统的集成必须同时满足以下四条硬门；测试、沙箱和预发布环境也属于真实外部系统：

1. **环境选择 fail-closed**
   - 目标环境必须显式配置并通过允许列表校验。
   - 缺失、未知或拼写错误必须停止初始化或执行；不得隐式回退到测试、沙箱、Mock、预发布或生产。
2. **配置值保持真实作用域**
   - 外部获得的值必须先判断属于协议、产品、环境、租户、账户还是请求作用域。
   - 只有协议级不变量可成为全局常量；单一示例、账号、租户或环境产生的值必须保留在对应可配置作用域。
3. **真实外部调用必须显式启用**
   - 联网测试和运维探测默认关闭。
   - 启用时必须显式提供目标地址、身份标识和凭据；源码、fixture、脚本和测试辅助不得内置可直接使用的真实系统默认值。
4. **新配置默认不产生业务流量**
   - 新建或迁移的外部集成配置默认不可运行。
   - 只有配置校验、凭据检查和必要验证门通过后，才能执行显式启用步骤。

Review 只回答四个问题：配置错误时是否停止；局部参数是否被提升为全局规则；是否存在打开开关即使用内置真实参数联网的路径；新配置是否会未经验证立即产生流量。具体供应商、表、枚举、字段和接口路径必须留在项目画像、项目经验或领域专项 Skill。
...
server-dev 透镜在 cap 主线的以下阶段被路由加载（`role-routing.md` 命中 `api/**`、`handlers/**`、`*.server.*`、`models/**`、`*.sql` 等）：
...
写入 `.cap/review/server-dev.md`（单写者，避免并发竞争），每行一个 finding：
