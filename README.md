...
> **状态**：v0.6.5，持续演进中。Skills 可以脱离 Cap Server 独立运行，支持 Codex、Claude Code 和 Cursor，不锁定单一模型、平台或公司环境。

## v0.6.5 更新

这一版本修稳了团队模式下最关键的经验闭环：

- 完整结构化经验直接进入知识库，不再被不必要的模型二次萃取阻断。
- 沉淀与离线重放共享稳定幂等身份，重复请求不会重复累计经验和指标。
- 后续任务只对实际采用的知识精确归因，并绑定来源 Task、冻结知识快照与最终 Gate 结果。
- 可以形成“第一次踩坑、第二次精确召回并首轮通过”的可核验证据链，而不只是统计知识被展示过。
