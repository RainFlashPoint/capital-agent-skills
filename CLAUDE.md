...
- **行为 dogfood**:把某个流程 skill 真跑在一个**通用 fixture/真实项目**上,对照判据查产出(如 cap-understand 产的 surface-map 是否过 Phase C 三条自检)。产出样例见 `examples/`。**别把内部项目产物 commit 进来**(`docs/dogfood/` 已 gitignore,只在本地跑)。
...
## 路径约定(给别的 agent)
跨 skill 引用的共享数据(role-routing / roles / templates / languages)**物理上只在 `skills/cap-flow/references/` 下**;verify 的 checks 在 `skills/cap-test/checks/`;release 的 targets 在 `skills/cap-release/targets/`。解析共享数据时按"相对 skills 根的 `cap-flow/references/...`"定位,不要当作相对调用方 skill 目录。
